            ...
            for row in open(file_name, 'r'):
                ...
                if line.startswith("T"):  # Lines is a Brat TAG
                    try:
                        parts = line.split("\t")
                        if len(parts) >= 2:
                            # Dividir la segunda parte por espacios
                            label_parts = parts[1].split()
                            
                            # Buscar los dos últimos elementos que sean números
                            start = None
                            end = None
                            ...
                            for i, part in enumerate(label_parts):
                                try:
                                    ...
                                    # Si no es un número, continuar
                                    continue
                            
                            # Si encontramos ambos números, procesar
                            if start is not None and end is not None:
                                phi_tags.append((start, end))
                            else:
                                print(f"WARNING: No se pudieron encontrar posiciones válidas en línea: {line}")
                                
                    except Exception as e:
                        print(f"ERROR! Error procesando línea '{line}' en documento '{file_name}': {str(e)}")
                            ...
                            for i, part in enumerate(label_parts):
                                try:
                                    ...
                                    if start is None:
                                        ...
                                        # Todo lo que está antes de estos dos números es la etiqueta
                                        tag_parts = label_parts[:i-1]
                                        break
                                except ValueError:
                                    # Si no es un número, es parte de la etiqueta
                                    continue
                            
                            # Si encontramos ambos números, procesar
                            if start is not None and end is not None:
                                # Unir todas las partes de la etiqueta
...
class EntityTypeMetrics:
    ...
    def print_metrics(metrics):
        """Print the metrics for each entity type."""
        print("{:<20}{:<10}{:<10}{:<10}".format("Entity Type", "Precision", "Recall", "F1"))
        print("-" * 50)
