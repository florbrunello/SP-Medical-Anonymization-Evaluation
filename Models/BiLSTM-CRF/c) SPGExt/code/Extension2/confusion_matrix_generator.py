...
sobre las particiones de desarrollo y test del dataset SPGExt
...
def generate_confusion_matrix_from_annotations(gold_dir, system_dir, output_prefix="confusion_matrix", partition="dev", cleaned=False):
    """
    Genera matriz de confusión del modelo BiLSTM-CRF sobre las particiones dev/test del dataset SPGExt.
    ...
    # Título específico para SPGExt con partición dinámica
    partition_name = "desarrollo" if partition == "dev" else "evaluación"
    titulo = f"Matriz de confusión del modelo BiLSTM-CRF\nsobre la partición de {partition_name} del dataset SPG extendido"
...
def main():
    parser = argparse.ArgumentParser(description="Genera matriz de confusión NER del modelo BiLSTM-CRF sobre las particiones dev/test del dataset SPGExt con 28 etiquetas en orden fijo")
