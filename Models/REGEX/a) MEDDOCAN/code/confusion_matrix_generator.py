"""
Script para generar matriz de confusión NER del modelo REGEX
sobre la partición de desarrollo del dataset MEDDOCAN
con las 28 etiquetas en orden fijo: B-*, I-*, O
Genera los vectores l_true.txt y l_pred.txt usando la misma metodología que el modelo REGEX
"""
...
def tokenize_text(text):
    """Tokeniza el texto de la misma manera que el modelo REGEX"""
    import re
    tokens = []
...
def generate_confusion_matrix_from_annotations(gold_dir, system_dir, output_prefix="confusion_matrix", partition="dev", cleaned=False):
    """
    Genera matriz de confusión del modelo REGEX sobre la partición de desarrollo del dataset MEDDOCAN.
    ...
    print("Generando vectores l_true y l_pred usando metodología del modelo REGEX...")
...
def main():
    parser = argparse.ArgumentParser(description="Genera matriz de confusión NER del modelo REGEX sobre las particiones dev/test del dataset MEDDOCAN con 28 etiquetas en orden fijo")
