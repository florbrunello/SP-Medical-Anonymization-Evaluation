"""
Script para generar matriz de confusión NER del modelo BiLSTM-CRF
sobre las particiones de desarrollo y test del dataset MEDDOCAN
con las 28 etiquetas en orden fijo: B-*, I-*, O
Genera los vectores l_true.txt y l_pred.txt usando la misma metodología que el modelo BiLSTM-CRF
"""
...
def tokenize_text(text):
    """Tokeniza el texto de la misma manera que el modelo BiLSTM-CRF"""
    import re
    tokens = []
...
def generate_confusion_matrix_from_annotations(gold_dir, system_dir, output_prefix="confusion_matrix", partition="dev", cleaned=False):
    """
    Genera matriz de confusión del modelo BiLSTM-CRF sobre las particiones dev/test del dataset MEDDOCAN.
    La matriz incluye las 28 etiquetas en orden fijo.
    
    Args:
        gold_dir: Directorio con anotaciones gold standard
        system_dir: Directorio con anotaciones del sistema
        output_prefix: Prefijo para archivos de salida
        partition: Partición del dataset ("dev" o "test")
        cleaned: Si es True, indica que se usan datos limpiados
    """
    ...
    print("Generando vectores l_true y l_pred usando metodología del modelo BiLSTM-CRF...")
    ...
    # Identificar las filas (entidades) cuya suma es mayor que 0 (es decir, aparecen en el dataset)
    rows_to_keep = np.where(cm.sum(axis=1) > 0)[0]
    # Filtrar la matriz y las etiquetas
    ...
    # Título específico para MEDDOCAN con partición dinámica
    partition_name = "desarrollo" if partition == "dev" else "evaluación"
    titulo = f"Matriz de confusión del modelo BiLSTM-CRF\nsobre la partición de {partition_name} del dataset MEDDOCAN"
    ...
    # Guardar matriz con nombre que incluye partición
...
def main():
    parser = argparse.ArgumentParser(description="Genera matriz de confusión NER del modelo BiLSTM-CRF sobre las particiones dev/test del dataset MEDDOCAN con 28 etiquetas en orden fijo")
