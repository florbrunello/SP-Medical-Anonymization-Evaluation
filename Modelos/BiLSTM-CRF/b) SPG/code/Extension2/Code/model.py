...
class NeuralModel(object):
    ...
    # Función MODIFICADA para etiquetas largas
    def show_confusion_plot(self, confusion, all_tags, epoch):
        fig, ax = plt.subplots(figsize=(20, 18))  # Aumentamos el ancho para más espacio
        
        # Gráfico de la matriz (aspecto cuadrado)
        cax = ax.matshow(confusion, cmap="viridis", aspect="equal")
        ...
        # Etiquetas del eje X (abajo) - Rotadas 90°
        ax.set_xticklabels(all_tags, rotation=90, ha="center", fontsize=9)  # Fuente un poco más pequeña
        
        # Etiquetas del eje Y (izquierda) - Ajustes clave para evitar corte
        ax.set_yticklabels(all_tags, fontsize=9, va="center")  # Alineación vertical centrada
        
        # Ajustar márgenes manualmente
        plt.subplots_adjust(
            left=0.3,    # Más espacio a la izquierda para etiquetas largas
            bottom=0.3,   # Espacio para etiquetas del eje X
            right=0.9,
            top=0.9
        )

        # Guardar
        f = f"{self.config.fig_confusionplot}_{epoch}.png"
        print(f"Saving confusion plot at: {f}")
        plt.savefig(f, bbox_inches="tight", dpi=300)  # bbox_inches evita recortes
        plt.close(fig)
